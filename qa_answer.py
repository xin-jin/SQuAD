...
def generate_answers(sess, model, dataset, rev_vocab):
    ...
    """
    answers = {}
    instnace_num = len(dataset[0])
    for instance in range(instnace_num):
        
        if instance%100==0:
            print('working at ',instance+1,' out of ',len(dataset[0]))
            
            
        (instance_s,instance_e) = model.answer(sess,dataset[0][instance],dataset[1][instance])
        
           
        ans_ids = dataset[0][instance][instance_s:(instance_e+1)]
        ans_list = [rev_vocab[x] for x in ans_ids]
        ans_str = ' '.join(ans_list)
        answers[dataset[2][instance]] = ans_str
    return answers
...
def main(_):
    ...
    context_data = [line_to_list(x) for x in context_data]
    question_data = [line_to_list(x) for x in question_data]
...
#    decoder = Decoder(config)
    ...
    with tf.Session() as sess:
        ...
        print('started the initialiation ....')
